...
Extrait des informations structurelles du code source dans des fichiers `.skt` ou JSON.
...
| `--verbose`      | bool   | `false`           | Afficher des informations de progression pendant le traitement                                  |

Le répertoire de sortie par défaut est `./skeleton` lorsqu'il n'est pas spécifié. En mode `inline`, la sortie est écrite sur stdout et aucun répertoire n'est utilisé. Avec `--format json`, la sortie en répertoire est écrite sous forme de `codeknit.json`.

## codeknit graph show

Génère une visualisation interactive de graphe en HTML de la structure du codebase.
...
Exécute des algorithmes d'**analyse de graphe** et génère un rapport `.skt` lisible par un LLM.
...
| `--god-threshold`         | int     | `15`                             | Nombre minimal d'arêtes de type *contains* pour signaler une **god class** ou une fonction **god** |
...
| `--propagation-cutoff`    | float64 | `0.05`                           | Probabilité minimale pour poursuivre la simulation de propagation des changements |
...
| `--workers`              | int    | `0` (NumCPU)              | Nombre maximal de goroutines de parsing concurrentes |
| `--verbose`              | bool   | `false`                   | Afficher des informations de progression         |
