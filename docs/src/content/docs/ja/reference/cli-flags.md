...
インタラクティブなターミナルUI（TUI）を起動し、利用可能なコマンドとオプションをガイドします。
...
| `--format`       | string | `skt`            | 出力フォーマット: `skt`または`json`                                                        |
...
| `--workers`      | int    | `0` (NumCPU)     | 最大同時パースgoroutine数                                                          |
| `--verbose`      | bool   | `false`          | 処理中に進捗情報を表示する                                                               |

出力ディレクトリは指定しない場合、`./skeleton`がデフォルトになります。`inline`モードでは、出力はstdoutに書き込まれ、ディレクトリは使用されません。`--format json`を使用すると、ディレクトリ出力は`codeknit.json`として書き込まれます。
...
| `--max-commits`          | int    | `2000`                    | 検査する最大コミット数                            |
| `--max-files-per-commit` | int    | `50`                      | より多くのファイルを変更するコミットを除外する              |
| `--min-cochanges`        | int    | `3`                       | 時間的結合の最小共有コミット数                        |
...
| `--rerank`         | bool   | `false`                       | セマンティックな近傍を見つけ、Ollama埋め込みを使用して候補を再ランク付けする（`ollama serve`および`ollama pull qwen3-embedding:0.6b`が必要） |
