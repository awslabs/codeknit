...
description: ファジーハッシュを使用して、ファイルや言語を超えた重複および近似重複コードを検出します。
---
...
`codeknit fingerprint` は、コードベース内のすべての関数、メソッド、変数、および型を分析し、以下に基づいて**正規化された構造的フィンガープリント**を計算します：
...
この正規化により、**名前変更されたコピー＆ペースト**、**些細なリファクタリング**、および**異なる言語での同等のロジック**でも、重複として検出できます。
