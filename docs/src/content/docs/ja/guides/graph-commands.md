...
| `--workers`      | `NumCPU`                         | 最大同時解析goroutine数      |
...
コードベースに対して構造的なグラフアルゴリズムを実行し、コード品質に関する洞察を含むLLMが読み取り可能な`.skt`レポートを出力します。
...
| `--god-threshold`         | `15`                            | ゴッドクラス/関数としてフラグを立てる最小contains-edge数         |
| `--max-inheritance-depth` | `5`                             | これより深い継承チェーンをフラグ                                 |
