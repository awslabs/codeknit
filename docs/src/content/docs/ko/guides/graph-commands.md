---
title: 그래프 명령어
description: 그래프 알고리즘을 사용하여 코드베이스 구조를 시각화하고 분석하세요.
---

codeknit은 코드베이스 구조를 이해하고 개선하는 데 도움이 되는 두 가지 강력한 그래프 명령어를 제공합니다: 대화형 시각화를 위한 `graph show`와 자동 구조 분석을 위한 `graph analyze`입니다.
...
이 명령어는 순환 의존성, 허브 심볼, 데드 코드, 갓 클래스, 아키텍처 병목 등 일반적인 코드 품질 문제를 감지합니다.
...
- 산탄총 수술(Shotgun Surgery) 감지
- 기능 선망(Feature Envy) 감지
- 안정적 의존성 원칙 위반
- 인터페이스 분리 원칙 위반
- 포함 계층 깊이
...
| `--max-inheritance-depth` | `5`                             | 이보다 깊은 상속 체인을 표시                     |
| `--top-n`                 | `30`                            | 순위가 매겨진 출력 섹션의 최대 항목 수; 0 = 무제한 |
