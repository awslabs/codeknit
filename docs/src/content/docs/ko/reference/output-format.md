...
`.skt`(skeleton) 형식은 `codeknit`에서 추출된 코드 구조를 표현하기 위해 사용되는 간결하고 사람이 읽을 수 있는 텍스트 형식입니다. 이는 LLM 소비와 구조 분석에 적합한 최소한의 형태로 심볼, 관계 및 메타데이터를 포함합니다.
...
- **signature**: 심볼의 이름과 타입 정보입니다. 심볼 유형에 따라 형식이 다릅니다:
  - `name` — 타입, 값, 모듈용
  - `name(params)` — 반환 타입이 없는 호출 가능 항목용
  - `name(params) -> returnType` — 반환 타입이 있는 호출 가능 항목용
...
- `receiver=*TypeName`: 메서드의 경우, 수신자 타입을 나타냄
...
`[dict]` 섹션은 `--minify` 플래그가 사용된 경우에만 나타납니다. 이 섹션은 짧은 딕셔너리 코드를 반복되는 문자열 토큰에 매핑하여 출력 크기를 줄입니다.

### 형식

각 행은 딕셔너리 코드(`d0`, `d1` 등)를 확장된 값에 매핑합니다:
