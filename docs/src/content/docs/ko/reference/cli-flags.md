...
| `--minify`       | bool   | `false`          | 사전 기반 출력 축소 활성화                                                               |
...
출력 디렉터리는 지정하지 않으면 `./skeleton`이 기본값입니다. `inline` 모드에서는 출력이 stdout으로 작성되며 디렉터리는 사용되지 않습니다. `--format json`을 사용하면 디렉터리 출력은 `codeknit.json`으로 저장됩니다.
...
| `--betweenness-threshold` | float64 | `0.001`                         | 보고할 최소 매개 중심성 값                             |
...
| `--rerank`         | bool   | `false`                       | Ollama를 통한 의미 임베딩을 사용하여 CTPH 후보 재순위 (`ollama serve` 및 `ollama pull qwen3-embedding:0.6b` 필요)      |
