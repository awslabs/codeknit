...
소스 코드에서 구조적 정보를 추출하여 `.skt` 파일로 저장합니다.
...
출력 디렉터리는 지정하지 않으면 `./skeleton`이 기본값입니다. `inline` 모드에서는 출력이 stdout으로 작성되며 디렉터리는 사용되지 않습니다.
...
| `--workers`               | int     | `0` (NumCPU)                    | 최대 동시 파싱 고루틴 수                                   |
| `--verbose`               | bool    | `false`                         | 처리 중 진행 정보 출력                                     |
| `--fan-threshold`         | int     | `10`                            | 허브 심볼로 플래그 지정할 최소 팬인 또는 팬아웃            |
| `--god-threshold`         | int     | `15`                            | god class/function으로 플래그 지정할 최소 contains-edge 수 |
| `--max-inheritance-depth` | int     | `5`                             | 이 값보다 깊은 상속 체인 플래그 지정                       |
| `--top-n`                 | int     | `30`                            | 순위 출력 섹션 제한; `0`은 제한 없음                       |
| `--betweenness-threshold` | float64 | `0.001`                         | 보고할 최소 betweenness centrality 값                      |
...
| `--min-similarity` | int    | `65`                          | 보고할 최소 유사도 백분율 (0–100)                                                                                          |
| `--max-similarity` | int    | `95`                          | 보고할 최대 유사도 백분율 (0–100)                                                                                          |
| `--show-all`       | bool   | `false`                       | 원시 토큰 데이터가 포함된 `[fingerprints]` 섹션 포함                                                                       |
| `--rerank`         | bool   | `false`                       | Ollama를 통해 의미론적 임베딩을 사용하여 CTPH 후보 재순위 지정 (`ollama serve` 및 `ollama pull qwen3-embedding:0.6b` 필요) |
