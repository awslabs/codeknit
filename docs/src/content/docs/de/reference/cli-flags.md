...
| `--edges`        | bool   | `false`           | Fügt den Abschnitt `[edges]` in die Ausgabe ein (standardmäßig deaktiviert, um Token zu sparen) |
...
| `--top-n`                 | int     | `30`                           | Begrenzt die Ranglisten-Ausgabebereiche; `0` bedeutet keine Begrenzung                          |
| `--betweenness-threshold` | float64 | `0.001`                        | Mindestwert der Betweenness-Zentralität für die Berichterstattung                               |
...
Ordnet Dateien nach Git-Verlauf und struktureller Bedeutung und meldet temporale Kopplung zwischen Dateien, die wiederholt gemeinsam geändert werden.
...
| `--min-similarity` | int    | `65`                          | Mindest-Ähnlichkeit in Prozent für die Berichterstattung (0–100)                                |
| `--max-similarity` | int    | `95`                          | Höchst-Ähnlichkeit in Prozent für die Berichterstattung (0–100)                                 |
| `--show-all`       | bool   | `false`                       | Fügt den Abschnitt `[fingerprints]` mit Roh-Tokendaten ein                                      |
| `--rerank`         | bool   | `false`                       | Findet semantische Nachbarn und ordnet Kandidaten mithilfe von Ollama-Einbettungen neu (erfordert `ollama serve` und `ollama pull qwen3-embedding:0.6b`) |
| `--model`          | string | `qwen3-embedding:0.6b`        | Ollama-Einbettungsmodell für die Verwendung mit `--rerank`                                      |
| `--collect-test`   | bool   | `false`                       | Testdateien in die Analyse einbeziehen                                                          |
| `--workers`        | int    | `0` (NumCPU)                  | Maximale Anzahl gleichzeitiger Parsing-Goroutinen                                              |
| `--verbose`        | bool   | `false`                       | Gibt Fortschrittsinformationen während der Verarbeitung aus                                     |
