...
description: 使用模糊哈希检测跨文件和语言的重复和近似重复代码。
---
...
| `--min-similarity`   | `65`                          | 报告的最低相似度百分比（0–100）                                                                                                                            |
| `--max-similarity`   | `95`                          | 报告的最高相似度百分比（0–100）                                                                                                                            |
...
| `--collect-test`     | `false`                       | 分析中包含测试文件                                                                                                                                         |
| `--workers`          | `NumCPU`                      | 最大并发解析 goroutine 数量（0 = 使用所有 CPU 核心）                                                                                                       |
| `--verbose`          | `false`                       | 处理过程中打印进度信息                                                                                                                                     |
...
- **指纹衡量的是结构而非含义**：高相似度得分意味着代码*看起来*相似，而非*做*相同的事情。始终审查两个符号。
- **对噪声结果使用 `--rerank`**：如果出现大量误报，启用语义重新排序以使用嵌入过滤匹配。
- **跳过短主体**：规范化标记少于 4 个的符号（例如简单的 getter）会被忽略以避免噪声。
