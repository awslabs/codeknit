...
codeknit 提供了现成的技能，可教会 AI 编码助手如何有效使用它。这些技能使助手能够提取代码结构、检测重复代码并执行结构分析，而无需手动提示。

## 技能概览

codeknit 提供了两个技能：

- **`codeknit-parse`**：教会助手提取代码结构（函数、类、方法、变量）及关系（调用、继承、包含）到 `.skt` 文件中。
...
使用安装助手将技能目录复制到助手的技能文件夹中。安装程序仅下载捆绑的技能文件，因此无需克隆仓库。

为 **Codex**、**Kiro** 和 **Claude Code** 安装：
...
- 读取并解释 `.skt` 输出文件，包括 `[symbols]`、`[edges]` 和可选的 `[dict]` 部分
...
3. 助手回答结构性问题：依赖关系、调用链、死代码
4. 为了获得更深入的洞察，助手运行 `codeknit graph analyze ./src` 并解释报告
...
- **对于结构性问题，始终读取 `.skt` 文件而非原始源代码**——它们以紧凑、可靠的格式包含提取的结构
- 使用 `codeknit graph analyze` 揭示代码质量问题，如循环依赖、枢纽符号和深度继承链
- 在大型重构前运行 `codeknit fingerprint` 以识别应合并的复制粘贴代码
- `.skt` 格式旨在高效利用 token，非常适合 LLM 上下文窗口
- 在处理大型代码库时，使用 `--minify` 进一步减少 token 使用量