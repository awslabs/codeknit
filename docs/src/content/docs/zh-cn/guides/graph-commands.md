...
`codeknit` 提供了两个强大的图命令，帮助您理解和改进代码库结构：`graph show` 用于交互式可视化，`graph analyze` 用于自动化结构分析。
...
该命令检测常见的代码质量问题，如循环依赖、枢纽符号、死代码、god 类和架构瓶颈。
...
| `--workers`                   | `NumCPU`                        | 最大并发解析 goroutine 数                         |
| `--verbose`                   | `false`                         | 在处理过程中打印进度信息                          |
| `--fan-threshold`             | `10`                            | 标记枢纽符号的最小扇入或扇出数                    |
| `--god-threshold`             | `15`                            | 标记 god 类/函数的最小包含边数                    |
