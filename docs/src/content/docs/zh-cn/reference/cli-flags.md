...
当未指定输出目录时，默认为 `./skeleton`。在 `inline` 模式下，输出写入到 stdout 且不使用目录。使用 `--format json` 时，目录输出将写入为 `codeknit.json`。
...
生成的 HTML 文件是自包含的，并会自动在默认浏览器中打开。
...
| `--god-threshold`            | int      | `15`                              | 标记 god 类/函数的最小包含边数量                       |
| `--max-inheritance-depth`    | int      | `5`                               | 标记超过此值的继承链                                   |
| `--top-n`                    | int      | `30`                              | 限制排名输出部分；`0` 表示无限制                       |
