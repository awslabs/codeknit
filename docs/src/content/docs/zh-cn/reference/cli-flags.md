...
| `--edges`          | bool     | `false`           | 在输出中包含 `[edges]` 部分（默认关闭以节省 tokens）                                    |
...
| `--workers`                   | int       | `0` (NumCPU)                        | 最大并发解析 goroutine 数量                            |
| `--verbose`                   | bool      | `false`                             | 在处理过程中打印进度信息                               |
| `--fan-threshold`             | int       | `10`                                | 标记中心符号的最小扇入或扇出值                         |
| `--god-threshold`             | int       | `15`                                | 标记 god class/function 的最小包含边数量               |
| `--max-inheritance-depth`     | int       | `5`                                 | 标记超过此值的继承链                                   |
...
使用 Git 历史和结构重要性对文件进行排名，并报告重复变更的文件之间的时间耦合。
...
| `--max-commits`               | int      | `2000`                      | 检查的最大提交数量                        |
| `--max-files-per-commit`      | int      | `50`                        | 排除变更文件数量超过此值的提交            |
...
| `--show-all`            | bool     | `false`                          | 包含 `[fingerprints]` 部分，其中包含原始 token 数据                                                                  |
| `--rerank`              | bool     | `false`                          | 查找语义邻居并使用 Ollama 嵌入重新排名候选（需要 `ollama serve` 和 `ollama pull qwen3-embedding:0.6b`）              |
