...
Thư mục đầu ra mặc định là `./skeleton` khi không được chỉ định. Trong chế độ `inline`, đầu ra được ghi ra stdout và không sử dụng thư mục. Với `--format json`, đầu ra thư mục được ghi dưới dạng `codeknit.json`.
...
Tệp HTML được tạo ra là tự chứa và sẽ tự động mở trong trình duyệt mặc định của bạn.
...
| Cờ                     | Kiểu   | Mặc định                   | Mô tả                                      |
| ---------------------- | ------ | ------------------------- | ------------------------------------------ |
| `-o`, `--output`       | string | `./skeleton/hotspots.skt` | Đường dẫn tệp đầu ra                       |
| `--format`             | string | `skt`                     | Định dạng đầu ra: `skt` hoặc `json`        |
| `--since`              | string | `12mo`                    | Cửa sổ lịch sử, ví dụ: `180d`, `12mo`, hoặc `2y` |
...
| `--min-cochanges`      | int    | `3`                       | Số lượng commit chung tối thiểu cho kết hợp thời gian |
...
| `--workers`            | int    | `0` (NumCPU)              | Số lượng goroutine phân tích đồng thời tối đa |
| `--verbose`            | bool   | `false`                   | Hiển thị thông tin tiến trình              |
...
Tạo các script hoàn thiện shell cho các shell được hỗ trợ.
