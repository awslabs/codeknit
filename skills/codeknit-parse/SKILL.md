...
The output directory defaults to `./skeleton` for `directory-flat` and `directory-tree` modes. Pass a second argument to override it. In `inline` mode no directory is created.
...
The output format has sections: `[symbols]`, `[edges]`, optionally `[errors]` and `[dict]`.

See [OUTPUT-FORMAT.md](OUTPUT-FORMAT.md) for the complete format reference with examples.

## Important: use .skt files for analysis, not source code

When performing structural analysis (dependency mapping, refactoring planning, architecture review, etc.), **always read the `.skt` skeleton files** instead of the raw source code. The `.skt` files contain the extracted structural information — symbols, relationships, line spans — in a compact format designed for exactly this purpose. Reading source files directly for structural questions wastes context and is less reliable.

Only read the actual source code when you need to inspect or modify implementation details (e.g. fixing a bug, reviewing logic, writing new code). For anything structural — "what calls what", "where is this class used", "show me the dependency graph" — the `.skt` files are the right tool.
